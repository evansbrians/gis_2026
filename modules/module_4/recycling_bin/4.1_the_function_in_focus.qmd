...
lesson_metadata(.dataset = c("earley.csv", "sites.csv", "populations_co2.rds", "spotted_lanternfly.rds")) %>%
  cat()
```
:::
...
2. Save your script file as "`scripts/function_in_focus_written_content.R`".
...
Note: The other operations that we have conducted in this class allow you to place your cursor anywhere within the code block and hit `Ctrl + Enter` (Windows) or `Cmd + Enter` (Mac) to run your code. When assigning a function to your global environment, however, you can only run the full code block if your cursor is placed anywhere *outside* of the curly braces. We will discuss why this happens soon!
...
... because the population data contain an `NA` value! 
...
Perhaps we do not always want to remove `NA` values though? For example, the presence of `NA` values are very important if we are calculating global CO~2~ emissions! 
