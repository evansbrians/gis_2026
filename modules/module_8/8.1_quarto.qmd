...
You can add instructions to the execution of the code block with chunk-option lines placed immediately below the opening code fence. For example, to remove the printed messages after attaching the tidyverse packages, you can use `#| message: false` (please add this to your document and render it):
...
We can see that, while `#| message: false` suppressed the printed messages from `library(tidyverse)`, rendering the code block still returned the code and the tibble it generated.

Sometimes we want to include only the output (e.g., for public reports or when showing your data to non-R-savvy advisors). To include only the output of our code, but not the code itself, add `#| echo: false` immediately below the opening code fence:
...
To display the code itself without running it, add `#| eval: false` immediately below the opening code fence:
...
.tara_style { font-family: Times; }
```
````
*Note on the above: We used* `#| echo: false` *because we do not want to show the reader our CSS! Also note that the period in* `.tara_style` *defines a uniquely identified class.*
...
While I prefer to place my CSS code chunk near the top of a document, you can actually place it anywhere. Styles defined by a CSS code chunk are applied across the whole document, regardless of the position of that chunk. For example, executing the following anywhere in the document ...
