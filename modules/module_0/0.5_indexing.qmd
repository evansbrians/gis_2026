...
A key trait of data objects is that their components are ordered -- in other words each of an object's components (e.g., values in an atomic vector) occupies a position within the object. In this lesson, we'll look at the foundational method of subsetting -- **indexing**. Indexing describes the process of subsetting an object by the position of a component of interest. We'll also look at how to subset a data frame by condition -- a process called **filtering** -- using the tidyverse function `filter()`.

In this lesson, you will learn:

* How to extract single or multiple values from atomic vectors, matrices, lists, and data frames using `[[]]` and `[]`
* The tidyverse alternative to `[[]]` for single-value extraction, `pluck()`
...
[{{< fa exclamation-triangle size=2x >}} Important!]{style="color: red;"} This lesson assumes that you have completed [Preliminary Lesson 4: Objects](0.4_objects.qmd){target="_blank"}. As the material covered herein will build on the content in that lesson, please do not proceed until its completion.
...
::: mysecret
{{< fa user-secret size=2x >}} [Of course, a matrix is still an atomic vector!]{style="font-size: 1.25em; padding-left: 0.5em;"}
...
::: mysecret
{{< fa user-secret size=2x >}} [Use double-brackets for extracting individual items!]{style="font-size: 1.25em; padding-left: 0.5em;"}
...
The *purrr* package (a **core tidyverse** package) gives us another way to extract a single value or list item -- the function `pluck()`. Rather than wrapping the object in brackets, we supply the object as the first argument and the position as the second:
...
This produces exactly the same result as `[[]]` -- it's simply function-call syntax instead of bracket notation. You'll see `pluck()` used often later in this course, especially for pulling a single item out of a list (e.g., one table out of a list of related tables), so it's worth getting comfortable with both forms now.

::: mysecret
{{< fa user-secret size=2x >}} [So which should I use, `[[]]` or `pluck()`?]{style="font-size: 1.25em; padding-left: 0.5em;"}

Either is fine for the single-value/single-item extraction we're doing here -- I use `pluck()` most often when it's one step within a piped chain of tidyverse functions, and `[[]]` most often when I'm working with an object on its own. We'll come back to this distinction, and to `pull()` (a close relative of `pluck()` for data frame columns specifically), when we cover subsetting data frames later in the course.
:::
...
::: now_you
{{< fa user-circle size=2x >}} [True or false: Best practice is to use double square brackets (`[[ ]]`) to extract a single value or list item and single brackets (`[ ]`) to extract more than one.]{style="padding-left: 0.5em;"}
...
<button id="p-0-5-02-check" class="btn btn-outline-dark btn-sm" type="button">Check My Order</button>
<button id="p-0-5-02-reset" class="btn btn-outline-dark btn-sm" type="button">Shuffle Again</button>
...
`r longmcq(c(answer = "A data frame is a recursive object, so its names are defined inside it; an atomic vector's names are a non-recursive attribute", "$ never needs quotes and [[ ]] always does", "It is an arbitrary style choice", "Data frames cannot be indexed with quotes"))`
:::
...
*Note: There isn't a tidyverse equivalent for this operation -- `filter()`, which we turn to next, is built specifically for data frames. When you need to filter a bare vector like `my_object`, the approach above is still the way to go!*

::: now_you
{{< fa user-circle size=2x >}} [True or false: To return the matching values when a logical test is placed inside `[]` (e.g., `my_object[my_object > 1]`), you must wrap the test in `which()`.]{style="padding-left: 0.5em;"}

`r torf(FALSE)`
:::

::: now_you
{{< fa user-circle size=2x >}} [Parsons Problem: Assemble the simplest way to subset `my_object` to values greater than 1. One line is an unneeded extra step that produces the identical result -- leave it in the trash.]{style="padding-left: 0.5em;"}

<div class="parsons-problem">
<div id="p-0-5-03-trash" class="sortable-code"></div>
<div id="p-0-5-03-sortable" class="sortable-code"></div>
<div style="clear: both;"></div>
<button id="p-0-5-03-check" class="btn btn-outline-dark btn-sm" type="button">Check My Order</button>
<button id="p-0-5-03-reset" class="btn btn-outline-dark btn-sm" type="button">Shuffle Again</button>
...
We can filter on a character column the same way -- or, piped (the form we'll use most often from here on):
...
* The condition has to go in the **row** position, *before* the comma -- put it after the comma (`my_data_frame[, my_data_frame$bunnies < 10]`) and R quietly does something else entirely (it tries to use your logical test to select *columns*, which is almost never what you meant).

`filter()` sidesteps both problems: there's no comma to place a condition around, and the condition is evaluated *inside* the data frame automatically, so you never have to write `my_data_frame$` (or `$` at all) inside of it.
:::
...
::: now_you
{{< fa user-circle size=2x >}} [Use indexing to extract the value in the first row and column of `my_data_frame`.]{style="padding-left: 0.5em;"}
...
::: now_you
{{< fa user-circle size=2x >}} [Use `filter()` to subset `my_list[[3]]` (*Hint: You could also use `pluck()` to extract this list item!*) to rows in `words` that contain the value "hello".]{style="padding-left: 0.5em;"}
...
::: now_you
{{< fa user-circle size=2x >}} [Parsons Problem: Reorder the lines below to assign `my_data_frame` into `my_list[[3]]`, then pluck it back out and filter it to rows where `words` is `"hello"`. One line doesn't belong -- leave it in the trash.]{style="padding-left: 0.5em;"}

<div class="parsons-problem">
<div id="p-0-5-01-trash" class="sortable-code"></div>
<div id="p-0-5-01-sortable" class="sortable-code"></div>
<div style="clear: both;"></div>
<button id="p-0-5-01-check" class="btn btn-outline-dark btn-sm" type="button">Check My Order</button>
<button id="p-0-5-01-reset" class="btn btn-outline-dark btn-sm" type="button">Shuffle Again</button>
...
window.parsonsProblems.push({
  id: "p-0-5-01",
  maxDistractors: 1,
  code: "my_list[[3]] &lt;-\n  my_data_frame\npluck(my_list, 3) |&gt;\n  filter(words == \"hello\")\nmy_list |&gt;\n  filter(words == \"hello\") #distractor"
});
</script>
:::

::: now_you
{{< fa user-circle size=2x >}} [Which of the following is TRUE about recursive data objects?]{style="padding-left: 0.5em;"}

`r longmcq(c("A matrix is a type of recursive object", answer = "A data frame is a type of recursive object", "Quotation marks must be used when calling names assigned within an object", "The values of recursive objects are stored within the object itself"))`
:::
...
* Use **double brackets `[[ ]]`** to extract a *single* value or list item, and **single brackets `[ ]`** to extract *one or more*. When in doubt, remember the rule of thumb: `[[ ]]` for one, `[ ]` for many.
...
* The tidyverse `pluck()` is an **equivalent to `[[ ]]`** for pulling out a single item, and it reads especially well inside a pipe -- reach for it when extraction is one step in a tidyverse chain, and `[[ ]]` when you're working with an object on its own.
* You can index by **name** just as easily as by position -- but names go **in quotes** (e.g., `my_object[["apple"]]`) because the name lives inside the object, not in your global environment.
* The **`$` operator** extracts a list item or data-frame column **without quotes**, because a data frame is a **recursive** object -- its names live inside it, like little pointers you can call directly. That same `$` does *not* work on atomic vectors or matrices: their names are just a flat attribute, so there you must fall back to `[["name"]]`.
* Use **`str()`** to reveal an object's structure -- its class, dimensions, names, and a peek at the values -- whenever you need to remind yourself what you're working with.
* A **logical test placed inside `[ ]`** already returns the `TRUE` positions, so wrapping it in `which()` is an unnecessary extra step. Think of `[ ]` as the word "where": `my_object[my_object > 1]` reads as "the values of `my_object` where they're greater than 1."
...
::: panel
* **Filtering**: The process of subsetting a data frame's rows by condition, most often with `filter()`
...
render_function_table("functions_0.5_indexing.csv")
```
