...
Warning! Be aware of how many names are in your global environment. Each name represents an object that is stored in your system's memory -- an accumulation of objects can greatly reduce performance.
:::
...
* The commas are syntax that separates the arguments of the function (*Note: commas are not functions*).
...
This is what's known as a **nested function** because `c()` is nested inside the parentheses of `mean()`. Remember the order of evaluation: `c()` must be evaluated before `mean()` can calculate the average.
