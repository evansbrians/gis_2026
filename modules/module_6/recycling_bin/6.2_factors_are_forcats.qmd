...
lesson_metadata(
  .dataset = "forcats_data.rds",
  .tables = "bird_measures"
) %>%
  cat()
```
:::
...
3. Open the script file `factors_are_forcats.R` (Windows: `Ctrl + O`; Mac: `Cmd + O`).
...
In this written portion of the tutorial, we will again attach the core packages in the tidyverse to our current session and read in and globally assign each of the list items in  `forcats_data.rds`:
...
Since `NA` values represent missing data, we *may* not want to include `NA` in our counts. We can remove `NA` `age` values from the data frame with `drop_na()`:
...
Again, to convert those "unknown" values to `NA`, we can modify them with `na_if()` prior to converting the data to a factor:
...
What if, however, we wanted to convert "unknown" values to `NA`? Let's go back to the distribution of unique values in our original vector:
...
... or explicitly define which values to transform to `NA`: 
...
However, any values without a match in our `case_when()` statement are assigned to the value `NULL`. As such, this could be written more simply as:
...
If we want to convert a value or values to `NA`, we assign those values to `NULL`:
