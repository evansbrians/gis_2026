...
lesson_metadata(.dataset = "birds_cicadas_lc.rds") %>%
  cat()
```
:::
...
3. Open the script file `categorize.R` (Windows: `Ctrl + O`; Mac: `Cmd + O`).
...
* Read in `data/processed/birds_cicadas_lc.rds`;
...
We can also add categories for missing values. By default, these will be classified as `NA`:
