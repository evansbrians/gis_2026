...
-   How to read data from different file types (*this section includes a fairly long video and written review of the video content*)
...
Reading data imports data from a file or remote source into the current R session. Please watch and code along with the video below. I review some key concepts that I hope you gathered from our previous lessons and break new ground with a description of the process of reading data into R.
...
For example, I am working in a project called `gis_in_r`. The path to my `gis_in_r` folder (the working directory of my project) is:
...
```
/Users/brianevans/gits/teaching/gis_in_r/data/raw
```

The `raw` data folder contains a *.csv* file called `portal.csv`. The full path to this file is:

```
/Users/brianevans/gits/teaching/gis_in_r/data/raw/portal.csv
```
...
read_csv("/Users/brianevans/gits/teaching/gis_in_r/data/raw/my_covid_garden.csv")
```
...
In Base R, we can set our working directory using the function `setwd()`. Before RStudio came along, this function was at the top of most of our scripts. It might have looked something like this (**Do not run!**):
...
setwd("/Users/brianevans/gits/teaching/gis_in_r")
...
::: mysecret
<i class="fas fa-user-secret" style="font-size:36px"></i>  [Avoid using `setwd()`!]{style="font-size: 20px"}

Using `setwd()` in your code is less portable than sharing code managed in RStudio projects. Doing so requires you to communicate to others (and your future self) to modify their working directory to match the location on their file systems. When you share a **project** with your collaborators, everything will already be in place for them to execute the code.

Even more importantly, you might change your mind about where you want to put your code or files. If you move your files to a new location, your code will no longer run until you modify the path within the `setwd()` function. Your code will be "broken" until you repair file paths. When you use projects, you can move your project anywhere you want in your file system and R will still be able to find your files.
:::

::: now_you
{{< fa user-circle size=2x >}} [Check your understanding]{style="padding-left: 0.5em;"}

You are working inside the course project. Which path should you use to read `my_covid_garden.csv`?

`r longmcq(c("/Users/student/gis_2026/data/raw/my_covid_garden.csv", answer = "data/raw/my_covid_garden.csv", "gis_2026/data/raw/my_covid_garden.csv", "setwd(data/raw/my_covid_garden.csv)"))`
:::

### Review: Read csv files

Tabular data are most often stored as a **CSV file** -- "CSV" stands for "comma-separated values." Data in this format are stored as plain-text files in which commas separate values, and each record (e.g., row in a data frame) is placed on a new line. A csv file is a comparatively simple, open, plain-text format that can be used widely across computer programs.

It is best to read in *.csv* files with the *readr* function `read_csv()` (*Note: Remember that the readr package is a part of the core tidyverse!*). If we are working in a project, we supply the relative file path inside of parentheses. Let's read in the file `my_covid_garden.csv` from the folder `data/raw` and assign the object to the name `my_garden`.
...
Whenever you use `read.csv()`, additional exploration steps are necessary to search for errors in your data (see Data exploration, below). It is best to avoid this function. While you should take a moment to validate and explore almost every data file when you read it for the first time, `read_csv()` saves you a couple of steps in the process!
:::
...
**RDS files** are the most straightforward file type to read in (but see the caveat associated with these files in "Writing data", below). RDS files are data sets that are written in, and written for, R.
...
Notice that the first column, `bed`, is an integer and `light_access` and `species` were read in as factors. When you read in an RDS file, the returned object contains all of the classes and attributes of the object when it was written.

::: now_you
{{< fa user-circle size=2x >}} [Check your understanding]{style="padding-left: 0.5em;"}

What is a key advantage of using an RDS file for an object that will remain in R?

`r longmcq(c("It can be opened by any spreadsheet program", "It always uses less memory than other formats", answer = "It preserves the object's classes and attributes", "It automatically corrects problems in the data"))`
:::
...
In the above, we notice that there are quite a few bad column names and that what are most likely the correct column names are printed on the second line of data. We can add a `skip = [number of rows]` argument to skip the first two rows of data:
...
::: mysecret
{{< fa user-secret size=2x >}}  [When working with course data, please avoid opening an .xlsx file with Excel!]{style="font-size: 20px"}

I recommend avoiding the temptation to peek at an Excel file in Microsoft Excel. That is because, if a file is saved, Excel can alter the values (e.g., change the encoding associated with a variable). When working with dates, this can lead to huge issues!
:::

::: now_you
{{< fa user-circle size=2x >}} [Check your understanding]{style="padding-left: 0.5em;"}

Reorder the lines below to read the `garden_life` worksheet and skip its first two rows. One line uses the wrong function. Leave it in the trash.

<div class="parsons-problem">
<div id="p-1-4-01-trash" class="sortable-code"></div>
<div id="p-1-4-01-sortable" class="sortable-code"></div>
<div style="clear: both;"></div>
<button id="p-1-4-01-check" class="parsons-btn parsons-btn-check" type="button">Check My Order</button>
<button id="p-1-4-01-reset" class="parsons-btn parsons-btn-reset" type="button">Shuffle Again</button>
<p id="p-1-4-01-feedback" class="parsons-feedback"></p>
</div>

<script>
window.parsonsProblems = window.parsonsProblems || []; window.parsonsProblems.push({ id: "p-1-4-01", maxDistractors: 1, code: "garden_life <-\n  readxl::read_excel(\n    \"data/raw/my_covid_garden.xlsx\",\n    sheet = \"garden_life\",\n    skip = 2\n  )\nread_csv(\"data/raw/my_covid_garden.xlsx\") #distractor" });
</script>
:::
...
This returned the number of rows (the first number) and columns (the second number) as an integer vector.
...
True or false: For a data frame, `length()` returns the number of rows.
...
The dplyr package, part of the tidyverse core, contains a set of functions that start with the prefix `slice_` that allow you to print a subset of the data (see `?slice`).
...
You can also look at summary statistics individually. This can be especially useful programmatically. I will not bore you with a lot of text to describe each one, but here are a few to try on your own (*see* `?[function_name]` *if you have any questions about what each does!*):
...
No matter how much I warn against this, you are going to use this function anyway, and when I see you do it during our help sessions, I will invariably complain -- I might even emit a high-pitched whiny squeal. The student response to my whining is usually something like "... but I like to see my data."
...
**Addendum**: I was recently working with a data frame that contained more than 20 columns. I ended up using `View()` because I needed to visually scan across many of those columns simultaneously. As such, it seems my warning above might be a little too absolute.
:::

::: now_you
{{< fa user-circle size=2x >}} [Check your understanding]{style="padding-left: 0.5em;"}

Match each function to the information it provides.
...
If you intend to share your vector outside of R, saving the vector as a text file is a great way to do it. To do so, use the `write_lines()` function. The first argument of the function is the data to write and the second argument is the relative file path:
...
-   The file is too big to fit in your memory

**As a final note on RDS files**: While *.RDS* files are incredibly useful, I typically only write data as *.RDS* files during intermediate steps of a data management pipeline. Because they are platform-specific, and thus have limited portability across software and systems, they should not be used for long-term data storage.
:::
...
```{r}
write_rds(my_list, file = "data/raw/temp.rds")
```
...
In this class we will *not* write files with `write.csv()`. Row numbers are rarely useful and, though you can supply an additional argument to avoid writing them, there is a better way ...
:::
...
No row numbers! Let's delete the file:
...
write_rds(my_garden, file = "data/raw/temp.rds")
...
1. You want to save a data frame for a collaborator who does not use R. Which format is the best choice among those introduced in this lesson?

`r longmcq(c(answer = "A CSV file written with write_csv()", "An RDS file written with write_rds()", "An R script containing the printed data", "A list written with write_rds()"))`

2. True or false: After writing an important file, reading it back into R is a useful way to verify the result.

`r torf(TRUE)`

3. A data frame contains 24 rows and 6 columns. What will `length()` return?

`r longmcq(c("24", answer = "6", "30", "The number of values in every column"))`

4. Parsons Problem: Reorder the lines below to write `my_garden` as a CSV file, read the file back into R, and then remove it. One line reverses the arguments supplied to `write_csv()`. Leave it in the trash.

<div class="parsons-problem">
<div id="p-1-4-02-trash" class="sortable-code"></div>
<div id="p-1-4-02-sortable" class="sortable-code"></div>
<div style="clear: both;"></div>
<button id="p-1-4-02-check" class="parsons-btn parsons-btn-check" type="button">Check My Order</button>
<button id="p-1-4-02-reset" class="parsons-btn parsons-btn-reset" type="button">Shuffle Again</button>
<p id="p-1-4-02-feedback" class="parsons-feedback"></p>
</div>

<script>
window.parsonsProblems = window.parsonsProblems || []; window.parsonsProblems.push({ id: "p-1-4-02", maxDistractors: 1, code: "write_csv(my_garden, \"data/raw/temp.csv\")\ncheck <- read_csv(\"data/raw/temp.csv\")\nfile.remove(\"data/raw/temp.csv\")\nwrite_csv(\"data/raw/temp.csv\", my_garden) #distractor" });
</script>
:::

## Take-home points

* Work inside an RStudio project and use paths that are relative to the project folder.
* Choose an importing function that matches the file type. Read the messages printed during import and supply additional arguments when the file needs special handling.
* Explore every data object after reading it. Check its structure, dimensions, ordering, unique values, and statistical summaries.
* Choose an output format based on how the file will be used and who needs to open it.
* Read important output files back into R. Confirm that the object was written as intended before continuing your analysis.
