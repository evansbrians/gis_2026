...
Spatial vector data represent discrete geographic **features** as **points**, **lines**, or **polygons**. A feature is a single thing that the data describe, such as one county or one sampling site. A point marks a single location, a line connects a sequence of locations, and a polygon encloses an area. As we discussed in ***3.1 What is a GIS?***, I will often refer to spatial vector data colloquially as "shapefiles," although an Esri Shapefile is one specific spatial vector file format.

In this tutorial, we will begin using R as a GIS by reading spatial vector data and preparing them for analysis. The written tutorial is complete on its own. The *optional* video at the end covers the same workflow with a different set of data. Material covered will include:
...
* The number of **fields**, the columns that describe each feature;
...
:::{class="mysecret"}
<i class="fa fa-user-secret" aria-hidden="true" style = "font-size: 150%; padding-right: 5px;"></i>We should always read the metadata carefully! The last bit of metadata, the CRS, is the most important. When we compare or combine simple features objects spatially, their CRSs must be equivalent. If not, we must **transform** one of them, recalculating its coordinates so that they describe the same locations in the other CRS.
:::
...
4. Parsons problem: Reorder the lines below to read in `counties.geojson` (with no metadata printed during the reading process), clean the field names, subset the features to the District of Columbia, subset the fields to `geoid`, and transform the object to the CRS of `census`. One line applies a function that an existing simple features object does not require. Leave it in the trash.
   ...
   <script>
   window.parsonsProblems = window.parsonsProblems || []; window.parsonsProblems.push({ id: "p-3-2-01", maxDistractors: 1, code: "st_read(\n  \"data/raw/shapefiles/counties.geojson\",\n  quiet = TRUE\n) %>%\n  clean_names() %>%\n  filter(state_name == \"District of Columbia\") %>%\n  select(geoid) %>%\n    st_as_sf(coords = c(\"long\", \"lat\")) %>% #distractor\n  st_transform(\n    st_crs(census)\n  )" });
   </script>
:::
