...
Now let's fill our polygons by population. The argument, `fill = ` specifies the interior color, as in *ggplot2*. Name the target field in quotes:
...
Let's add the Esri World Imagery basemap below our census polygons:

```{r}
#| fig-alt: "Semi-transparent DC population polygons over Esri World Imagery satellite tiles in Web Mercator."
...
Each visual variable has its own legend settings. For polygon fill, use `fill.legend = ` with `tm_legend()`:
...
```{r}
#| fig-alt: "Capital Bikeshare locations drawn over the DC population polygons and Esri World Imagery basemap, with a Population legend and map title."
...
tm_basemap("Esri.WorldImagery") +
  ...
  # Add the bikeshare stations layer:
