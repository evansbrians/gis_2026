...
Each name that we supply specifies one level deeper, so a single call reaches the [common_name]{.mono} column of the [birds]{.mono} table from within the list:
...
1. You have read [district_birds.rds]{.mono} into a list named `district_birds`. A function has stopped with the message ``must be a data frame, not a `list` ``. You passed it `district_birds["birds"]`. What does `class(district_birds["birds"])` return?
...
3. Parsons Problem: Reorder the lines below to build a character vector of the common names in the [birds]{.mono} table of `district_birds`. One line reaches for an item in a list without quoting its name. Leave it in the trash.
...
`select()` subsets the data frame and returns a data frame with one column. `pull()` extracts the column and returns the character vector stored inside it. When a function expects a vector and you hand it a one-column data frame, this is almost always the reason.
:::
...
For example, perhaps we want capture records for birds caught in 2018. Capture dates live in `visits`, and `visit_id` connects the two tables -- the **primary key** of `visits` and a **foreign key** in `captures`.
...
* Subsetting returns an object of the parent's class with fewer children; extraction returns what is held inside the parent -- a child object, or metadata about the parent itself. When a function tells you it expected a vector and you handed it a one-column data frame, this is almost always why.
...
3. You have read [district_birds.rds]{.mono} into a list named `district_birds`. Which call returns the [captures]{.mono} table?

   `r longmcq(c(answer = 'pluck(district_birds, "captures")', "pull(district_birds, captures)", "select(district_birds, captures)", "slice(district_birds, captures)"))`

4. Which call removes the non-adjacent columns `capture_id` and `color_combo` from `captures`?

   `r longmcq(c("select(captures, !capture_id, !color_combo)", answer = "select(captures, !c(capture_id, color_combo))", "select(captures, !capture_id:color_combo)", "select(captures, c(!capture_id, color_combo))"))`
