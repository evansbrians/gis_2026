...
1. A table of iNaturalist observations repeats the class, order, and family of each species on every row. Which pair of functions would you use to split the taxonomy into its own table?

`r longmcq(c(answer = "select(), then distinct()", "unite(), then separate_wider_delim()", "pivot_wider(), then count()", "bind_cols(), then left_join()"))`
