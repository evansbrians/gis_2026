...
Converting those columns to numeric is a **data cleaning** step. Cleaning corrects the *content* of a dataset, which includes classes, units, misspellings, impossible values, and missing data codes. Tidying corrects the *structure* of a dataset, which is what the three rules govern. A dataset can be perfectly tidy and full of errors, and it can be scrupulously clean and completely untidy. The distinction matters because:

* Tidying and cleaning are done for different reasons
* Tidying and cleaning are typically done at different points in a workflow
* A tidy dataset is far easier to clean than a messy one.

Now that each variable has a column, we can convert all four of them in one step. The *dplyr* function `across()` applies the same change to a selection of columns, and we call it inside `mutate()`. Within `across()` we supply the following arguments:

* The columns that we would like to modify
* `~` to initiate a formula
* The function to apply to the columns
* `.x` as a variable that indicates the column name
...
```{r}
observations %>%
  drop_na(temperature_min, temperature_max) %>% 
  summarize(
    mean_min = mean(temperature_min),
    mean_max = mean(temperature_max),
    .by = station
  )
```
...
observations %>%
  ...
  summarize(
    max_temperature =
      max(temperature_max, na.rm = TRUE),
    .by = name
  )
```
...
4. Parsons Problem: Reorder the lines below to read `messy_weather.csv` and stack its day columns into a `day` column and a `value` column. One line assigns the column names to the wrong destination. Leave it in the trash.
   ...
   <script>
   window.parsonsProblems = window.parsonsProblems || []; window.parsonsProblems.push({ id: "p-2-6-01", maxDistractors: 1, code: "read_csv(\"data/raw/messy_weather.csv\") %>%\n  pivot_longer(\n    march_1:march_31,\n    names_to = \"day\",\n    values_to = \"value\"\n  )\n    names_to = \"value\", #distractor" });
   </script>
:::
