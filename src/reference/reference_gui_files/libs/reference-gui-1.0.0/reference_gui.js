...
(function () {
  ...
  document.addEventListener("input", function (event) {
    if (event.target.closest(".gui-editor")) markDirty();
  });
  ...
  document.addEventListener("keydown", function (event) {
    ...
    var editor = document.activeElement
      ? document.activeElement.closest(".gui-editor")
      : null;
    if (!editor) editor = document.querySelector(".tab-pane.active .gui-editor");
    if (!editor) return;
    var save = editor.querySelector(".gui-save");
    if (!save) return;
    event.preventDefault();
    save.click();
  });
